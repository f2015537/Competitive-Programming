...
void solve() {
  ...
  for(int i = 1; i < n; ++i){
    FOR(j,1,10){
      dp[i][j] = mod_add(dp[i][j], dp[i-1][j], MOD2);
      if(j >= 2) dp[i][j] = mod_add(dp[i][j], dp[i-1][j-1], MOD2);
      if(j <= 8) dp[i][j] = mod_add(dp[i][j], dp[i-1][j+1], MOD2);
    }
  }

  int ans = 0;

  FOR(j,1,10) ans = mod_add(ans, dp[n-1][j], MOD2);
