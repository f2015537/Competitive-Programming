...
void solve() {
  ...
  vi dp(n+1, -INF);
  dp[0] = 0;
  //dp[i] -> maximum subarray sum over all subarrays of length i
  for(int i = 0; i < n; ++i){
    //For every length, let's store the maximum subarray sum
    int sum = 0;
    for(int j = i; j < n; ++j){
      sum += v[j];
      int len = j-i+1;
  ...
  vi ans(n+1);
  //ans[i] -> maximum subarray sum after applying exactly i operations

  for(int i = 0; i <= n; ++i){
    for(int j = 0; j <= n; ++j){
      ans[i] = max(ans[i], dp[j] + x*min(i,j));
    }
  }
  each(x,ans) cout<<x<<" ";
  cout<<"\n";
