...
void solve() {
  ...
  vvi v(m,vi (3)); // {coordinate,cost,index}
  ...
  auto cmp = [&](vi& v1, vi& v2){ //comparison on the basis of cost only
    return v1[1] < v2[1];
  };
  ...
  auto cmp2 = [&](vi& v1, vi& v2){//comparison on the basis of x coordinate only
    return v1[0] < v2[0];
  }; 
  sort(all(v), cmp2);
