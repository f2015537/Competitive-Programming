...
void solve() {
  ll hc,dc; //Char health and attack
  cin>>hc>>dc;
  ll hm,dm; //Monster health and attack
  cin>>hm>>dm;
  ll k,w,a;
  cin>>k>>w>>a;

  //Let's use all k coins

  bool ans = false;
  for(ll i = 0; i <= k; ++i){
    //Invest i coins in attacks
    ll attack_char = dc + i * w;

    //Invest k-i coins in health
    ll health_char = hc + (k-i) * a;

    //Rounds char lasts
    ll x = ceil_div(health_char, dm);

    //Rounds monster lasts
    ll y = ceil_div(hm,attack_char);

    ans |= x >= y;

    if(ans) {
      dbg(x,y,attack_char,hm);
      cout<<"YES\n";
      return;
    }
  }
