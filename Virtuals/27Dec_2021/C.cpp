...
void solve() {
  ...
  // int index = 0
  F0R(i,n-1){
    mx = max(mx, s[i]-'0'+s[i+1]-'0');
  }

  dbg(mx);

  // If mx < 10 replace first first 2 characters of string with their sum
  string ans;
  if(mx < 10){
    int x = (s[0] - '0') + (s[1] - '0');
    char c = '0' + x;
    ans.pb(c);
    FOR(i,2,n)  ans.pb(s[i]);
    cout<<ans<<"\n";
    return;
  }

  // Else find rightmost index where s[i] + s[i-1] >= 10
  int left,right;
  for(int i = n-1; i >= 1; --i){
    int x = (s[i] - '0') + (s[i-1] - '0');
    if(x >= 10){
      ...
      int a = x % 10, b = x / 10;
      char c = '0' + a, d = '0' + b;
      for(int i = 0; i < left; ++i) ans.pb(s[i]);
      ans.pb(d); ans.pb(c);
      for(int i = right + 1; i < n; ++i)  ans.pb(s[i]);
      cout<<ans<<"\n";
      return;
    } 
  }
