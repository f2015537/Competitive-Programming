...
void solve() {
  ...
  //dp[i][j] -> minimum time taken from the starting city to the ith sign if we 
  // remove at most j signs

  F0R(i,k+1)  dp[0][i] = 0;
  FOR(i,1,n+1)  dp[i][0] = dp[i-1][0] + (d[i]-d[i-1])*a[i-1];
  dbg(dp); 

  FOR(i,1,n+1){
    FOR(j,1,k+1){
      //dp[i][j] -> minimum time taken from the starting city to city i
      //if we remove at most j signs
      for(int prev = i-1; prev >= 0; --prev){
        int rem = j - i + prev + 1;
        if(rem < 0) break;
        dp[i][j] = min(dp[i][j],dp[prev][rem] + (d[i]-d[prev])*a[prev]);
      }
    }
  }
  dbg(dp);
