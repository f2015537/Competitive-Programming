...
void solve() {
  ...
  ll cost = 0;
  for(ll i = 0; i < ceil_div(n,2); ++i){
    for(ll j = 0; j < ceil_div(m,2); ++j){
      vl temp;
      temp.pb(v[i][j]);
      if(j != m-1-j)  temp.pb(v[i][m-1-j]);
      if(i != n-1-i)  temp.pb(v[n-1-i][j]);
      if(j != m-1-j && i != n-1-i)  temp.pb(v[n-1-i][m-1-j]);
      dbg(i,j,temp);
      ll mc = LLONG_MAX;
      each(val, temp){
        ll cost = 0;
