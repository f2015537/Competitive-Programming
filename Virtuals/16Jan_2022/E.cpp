...
void solve() {
  ...
  set<pi> st; // <frequency, number>

  each(p,hsh) st.ins(mp(p.S,p.F));

  dbg(st);

  while(sz(st) > 1){ // O(nlgn)
    auto it1 = beg(st);
    int num1 = it1->S;
    int freq1 = it1->F;
    auto it2 = --en(st);
    ...
    dbg(freq1,num1);
    dbg(freq2,num2);
    st.erase(mp(freq1,num1));
    st.erase(mp(freq2,num2));
    if(freq1 > 1) st.insert(mp(freq1-1,num1));
    if(freq2 > 1) st.insert(mp(freq2-1,num2));3
  }
