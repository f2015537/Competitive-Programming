...
vvi g(N);
vi par1(N),par2(N),sz1(N),sz2(N);

void make(int v, vi& par, vi& size){
  par[v] = v;
  size[v] = 1;
}

int find(int v, vi& par){
  if(par[v] == v)  return v;
  return par[v] = find(par[v], par); //Path compression
}

void Union(int a, int b, vi& par, vi& size){
  a = find(a,par);
  b = find(b,par);
...
void solve() {
  ...
  sz1.rsz(n+1);
  sz2.rsz(n+1);
  FOR(i,1,n+1)  make(i,par1,sz1);
  FOR(i,1,n+1)  make(i,par2,sz2);
