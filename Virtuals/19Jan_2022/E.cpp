...
void solve() {
  ...
  F0R(i,k)  cin>>v[i].F, a1[v[i].F]++;
  F0R(i,k)  cin>>v[i].S, b1[v[i].S]++;
  //a1[x] -> Number of pairs that have x as first element
  //b1[x] -> Number of pairs that have x as second element
  ll ans = 0;
  each(pair, v){
    ans += k - (a1[pair.F] + b1[pair.S] - 1);
  }
  cout<<ans/2<<"\n";
}
