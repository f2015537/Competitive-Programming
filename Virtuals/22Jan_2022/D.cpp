...
void solve() {
  ...
  for(int i = n; i >= 1; --i){
    ...
    while(l <= n){
      int r = l;
      while(r <= n and pre[r]-pre[l-1] < intended) r++;
      if(r <= n) temp.pb(pre[r] - pre[l-1]);
      l = r+1;
    }
    //All elements in temp should be equal to intended
    //size of temp should be equal to i
    bool ok = sz(temp) == i;
    each(x,temp)  ok &= x == intended;
    if(ok){
      cout<<n-i<<"\n";
      return;
    }
