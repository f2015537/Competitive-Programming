...
int spruce(int i, int j, int level){//returns number of spruces with top at i,j
  if(i >= n)  return 0;
  int l = j-level+1;
  int r = j+level-1;
  int len = r-l+1;
  bool ok = l >= 1 and r <= m and pre[i][r] - pre[i][l-1] == len;
  if(ok)  return 1 + spruce(i+1,j,level+1);
  return 0;
}
