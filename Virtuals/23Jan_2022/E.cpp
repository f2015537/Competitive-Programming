...
void paint(int i, int j, int mx){
  temp[i][j] = '*';
  int p = i-1, q = j-1, r = j+1;
  rep(mx){
    temp[p][q] = '*';
    temp[p][r] = '*';
    p--;
...
int fun(int i, int j){//returns the maximum possible size of tick that can be drawn with (i,j) as center
  int ct = 0;
  for(int p = i-1, q = j-1, r = j+1; p >= 0 and q >= 0 and r < m; p--,q--,r++){
    if(grid[p][q] == '*' and grid[p][r] == '*') ct++;
    else break;
  }
  return ct;
}
...
void solve() {
  ...
  grid.clear();
  temp.clear();
  grid.rsz(n, vector<char> (m));
  temp.rsz(n, vector<char> (m, '.'));
