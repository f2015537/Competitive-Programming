...
void solve() {
  ...
  FOR(i,1,n+1)  cin>>v[i];
  vvl dp(n+1, vl(2));
  //dp[i][0] -> maximum sum that can be generated using prefix upto index i such that last element is added
  //dp[i][1] -> maximum sum that can be generated using prefix upto index i such that last element us subtracted
  FOR(i,1,n+1){
    dp[i][0] = max(dp[i-1][0], dp[i-1][1] + v[i]);//Add last element
    dp[i][1] = max(dp[i-1][1], dp[i-1][0] - v[i]);//Subtract last element
  }
  dbg(dp);
