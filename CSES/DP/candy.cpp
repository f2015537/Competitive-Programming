...
void solve() {
  ...
  dbg(candies);
  F0R(i,n){
    ll a = candies[i].F, b = -candies[i].S;
    int pos = lb(all(wagons),b) - beg(wagons);
    if(pos == sz(wagons)){
      wagons.pb(b);
    }
    else{
    ...
    output.pb({{(a+b)/2,(a-b)/2}, pos+1});
  }
  cout<<sz(wagons)<<"\n";
  each(p,output) cout<<p.F.F<<" "<<p.F.S<<" "<<p.S<<"\n";
}
